        ...
        for product in state.order_depths:
            ...
            if product == "AMETHYSTS":
                ...
                if idx == 0:
                    ...
                    if self.attributes['orchids_supply_spike_cnt'] < 3:
                        self.attributes['orchids_model'] = 'fallback'
                ...
                if self.attributes['orchids_model'] == 'default':
                    ...
                    active_buy_price = int(ext_sell_price - 1.5)
                    active_sell_price = int(ext_buy_price + 1.5) + 1
                    ...
                    active_buy_price = int(round(ext_sell_price, 0)) - 1
                    active_sell_price = int(round(ext_buy_price, 0)) + 2
                    for p, _ in sorted_buy_orders:
                        if p > ext_buy_price:
                            continue
                        else:
                            active_buy_price = min(p + 1, active_buy_price)
                            break
                    for p, _ in sorted_sell_orders:
                        if p < ext_sell_price:
                            continue
                        else:
